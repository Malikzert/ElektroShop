...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'delete_address') {
            ...
            // Mengubah query DELETE: Hanya menggunakan id_alamat
            $stmt = $conn->prepare("DELETE FROM alamat_pengiriman WHERE id_alamat = ?");
            if ($stmt) {
                // Mengubah bind_param: Hanya mengikat id_alamat
                $stmt->bind_param("i", $id_alamat);
                if ($stmt->execute()) {
                    if ($stmt->affected_rows > 0) {
                        echo json_encode(['status' => 'success', 'message' => 'Alamat berhasil dihapus.']);
                    } else {
                        // Pesan ini masih relevan jika id_alamat tidak ditemukan
        ...
        // Handle Edit Address (Tidak ada perubahan di sini)
